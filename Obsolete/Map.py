...
import Constants
import Engine.Fov
import Engine.SoundEffects
import Engine.xp_loader
import GameState
import Utils
import libtcodpy as libtcod
from Entities import Entity, Components
...
def load_prefab(x, y, key, rotation):
    ...
    xp_data = Engine.xp_loader.load_xp_string(raw_data)
    ...
    level_map = Engine.xp_loader.load_layer_to_map(level_map, map_x, map_y, xp_data['layer_data'][0],
                                                   rotation=rotation)
    if len(xp_data['layer_data']) >= 2:
        xp_objects = Engine.xp_loader.load_layer_to_objects(level_map, map_x, map_y, xp_data['layer_data'][1],
                                                            rotation=rotation)
    ...
    if len(xp_data['layer_data']) >= 3:
        level_map = Engine.xp_loader.load_layer_to_map_cosmetic(level_map, map_x, map_y, xp_data['layer_data'][2],
                                                                rotation=rotation)
    else:
        # print "3 is None"
        pass
...
def closest_monster(max_range):
    ...
    player = GameState.get_player()
    for object in get_visible_objects():
        if object.fighter and not object == player and Engine.Fov.is_visible(obj=object):
...
def target_tile(max_range=None):
    ...
    while True:
        ...
        if (mouse.lbutton_pressed and Engine.Fov.is_visible(x, y) and
