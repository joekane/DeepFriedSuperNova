...
class _QuadTree:
    ...
    def _remove_SLOW(self, item, pos):
        """
        Returns:

        - True if item was found and removed
        - False if item was not found or could not be removed
        """
        for child in self.children:
            child._remove(item)
        print "remove loop...."
    ...
    def _remove(self, item, bbox, results=None):
        rect = bbox
        if results is None:
        ...
        if len(self.children) > 0:
            if rect[0] <= self.center[0]:
                if rect[1] <= self.center[1]:
                    self.children[0]._remove(item, rect, results)
                if rect[3] > self.center[1]:
                    self.children[1]._remove(item, rect, results)
            if rect[2] > self.center[0]:
                if rect[1] <= self.center[1]:
                    self.children[2]._remove(item, rect, results)
                if rect[3] > self.center[1]:
                    self.children[3]._remove(item, rect, results)
