...
def initialze():
    ...
    noise_field = libtcod.noise_new(2, 0.5, 0.7, rnd)  # 0.5f  and 2.0f
    ...
    for y in range(Constants.MAP_CONSOLE_HEIGHT):
        for x in range(Constants.MAP_CONSOLE_WIDTH):
            libtcod.heightmap_set_value(height_map, x, y, get_noise_value(x, y, 5))

    libtcod.heightmap_normalize(height_map, 0.0, 1.0)
    libtcod.heightmap_add_hill(height_map, 40, 40, 6, 0.8)
    libtcod.heightmap_rain_erosion(height_map, 5000, 1.0, 0.0)
...
def get_noise_value(x, y, scale=10):
    ...
    # pre_value = libtcod.noise_get(Noise.noise_field, (nx, ny) , libtcod.NOISE_PERLIN)
    pre_value = libtcod.noise_get_fbm(noise_field, (nx, ny), 8, libtcod.NOISE_DEFAULT)
    # pre_value = libtcod.noise_get_turbulence(Noise.noise_field, (nx, ny), 8, libtcod.NOISE_DEFAULT)
    return pre_value
